...
## Procedural soft grain textures + per-layer height bands + swirl.
...
# Macro/micro wind: start at 0, shorter holds, mid-biased targets
...
const MACRO_START_RAMP := 2.5  # seconds to ease from 0 into first target
...
func _ready() -> void:
	...
	# Start at 0 and ramp into first mid-biased target
	_macro_from = 0.0
	_macro_current = 0.0
	...
	_layers.append(_make_layer({
		...
		"quad_size": 5.2, "modulate": Color(1.0, 0.12, 0.08, 1.0),
	...
	_layers.append(_make_layer({
		...
		"quad_size": 4.6, "modulate": Color(1.0, 0.35, 0.12, 1.0),
	...
	_layers.append(_make_layer({
		...
		"quad_size": 3.8, "modulate": Color(0.15, 1.0, 0.22, 1.0),
	...
	_layers.append(_make_layer({
		...
		"quad_size": 3.4, "modulate": Color(0.4, 1.0, 0.55, 1.0),
	...
	_layers.append(_make_layer({
		...
		"quad_size": 3.2, "modulate": Color(0.15, 0.75, 1.0, 1.0),
...
func _make_grain_texture(size: int, softness: float, irregularity: float) -> ImageTexture:
	...
	for y in size:
		for x in size:
			...
			img.set_pixel(x, y, Color(1.0, warm, warm * 0.72, a))
...
func _setup_layer_mesh(L: Layer) -> void:
	...
	match L.name:
		"heavy_low":
			tex_size = 24; soft = 0.52; irreg = 0.38; L.multi.z_index = 1
		"heavy_mid":
			tex_size = 22; soft = 0.55; irreg = 0.34; L.multi.z_index = 2
		"main":
			tex_size = 22; soft = 0.60; irreg = 0.28; L.multi.z_index = 3
		"main_high":
			tex_size = 20; soft = 0.62; irreg = 0.26; L.multi.z_index = 4
		"fine":
			tex_size = 18; soft = 0.58; irreg = 0.22; L.multi.z_index = 5
...
func _update_macro(delta: float) -> void:
	if _macro_lerp_t < 1.0:
		...
		if _macro_from <= 0.001:
			lerp_dur = MACRO_START_RAMP
