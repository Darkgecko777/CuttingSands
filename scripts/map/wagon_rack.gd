...
static func fill(grid: GridContainer, units: Array, on_unit: Callable) -> void:
	...
	for unit in units:
		...
		for part in span:
			grid.add_child(_cell(good_id, ghost, part == 0, on_unit))
			packed += 1
	for _i in maxi(0, GameState.caravan_capacity - packed):
		grid.add_child(_empty())


static func _cell(good_id: String, ghost: bool, head: bool, on_unit: Callable) -> Button:
	var cell := Button.new()
	cell.custom_minimum_size = Vector2(72, 52)
	cell.text = GameState.get_good_name(good_id)
	cell.tooltip_text = "%s  ·  %d cell  ·  %d mass" % [GameState.get_good_name(good_id), CargoMath.size_of(good_id), CargoMath.mass_of(good_id)]
