...
## Main Field shell: catalog rail + panned map + context panel.
...
func _refresh_header() -> void:
	...
	status_label.text = "Scrubstone %d    Cargo %d/%d" % [
		GameState.scrubstone, GameState.cargo_used(), GameState.caravan_capacity
	]
...
func _set_category(cat: int) -> void:
	...
	for key in _cat_buttons.keys():
		var btn: Button = _cat_buttons[key]
		btn.button_pressed = (key == cat)
	match cat:
		Catalog.CARAVANS:
			catalog_title.text = "Caravans"
		Catalog.AGENTS:
			catalog_title.text = "Agents"
		Catalog.SETTLEMENTS:
			catalog_title.text = "Settlements"
		Catalog.REPORTS:
			catalog_title.text = "Reports"
	_refresh_catalog()
		...
		Catalog.SETTLEMENTS:
			for city_id in GameState.CITIES.keys():
				items.append({
					"kind": "settlement",
					"id": city_id,
					"name": GameState.get_settlement_name(city_id),
				})
		Catalog.REPORTS:
...
func _show_settlement(city_id: String) -> void:
	...
	travel.text = "You are here" if here else "Travel here"
	travel.disabled = here
...
func _build_market() -> void:
	for good_id in GameState.GOODS.keys():
		...
		price.custom_minimum_size = Vector2(40, 0)
		row.add_child(price)
		var stock := Label.new()
...
func _clamp_map() -> void:
	...
	if MAP_SIZE.x > view.x:
		pos.x = clampf(pos.x, view.x - MAP_SIZE.x, 0.0)
	else:
		pos.x = (view.x - MAP_SIZE.x) * 0.5
	if MAP_SIZE.y > view.y:
		pos.y = clampf(pos.y, view.y - MAP_SIZE.y, 0.0)
	else:
		pos.y = (view.y - MAP_SIZE.y) * 0.5
